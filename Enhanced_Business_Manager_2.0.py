# business_manager.py
# Enhanced with employee availability, scheduling GUI, work library, employee editing/deletion (edits in pop-up), and sidebar + dynamic viewer GUI
...
# GUI Functions (views in content_frame, edits in pop-up Toplevel)
def clear_content(frame):
    for widget in frame.winfo_children():
        widget.destroy()

def add_employee_gui(manager, content_frame):
    ...
    def submit():
        ...
        try:
            ...
            clear_content(content_frame)  # Clear after submit
        except ValueError:
    ...
    tk.Label(content_frame, text="Availability (e.g., 2026-01-25:True,2026-01-26:False):").pack()
    avail_entry = tk.Entry(content_frame)
    avail_entry.pack()
...
def edit_employee_gui(manager, emp_index, refresh_callback):
    ...
    def submit():
        ...
        try:
            ...
            if avail_str:
                ...
                for pair in pairs:
                    date, avail = pair.split(':')
                    emp.availability[date.strip()] = avail.strip().lower() == 'true'
    ...
    avail_str = ','.join(f"{date}:{avail}" for date, avail in emp.availability.items())
    avail_entry.insert(0, avail_str)
    avail_entry.pack()

    tk.Button(window, text="Submit", command=submit).pack()

def view_employees_gui(manager, content_frame):
    clear_content(content_frame)
    ...
    def edit_selected(event):
        ...
        if selected:
            ...
            edit_employee_gui(manager, emp_index, refresh_list)  # Pass refresh as callback
...
def add_equipment_gui(manager, content_frame):
    clear_content(content_frame)
...
def add_work_item_gui(manager, content_frame):
    clear_content(content_frame)
...
def view_projects_schedules_gui(manager, content_frame):
    clear_content(content_frame)
    ...
    def show_schedule(event):
        ...
        if selected:
            ...
            # Clear and show weekly calendar in content_frame
            clear_content(content_frame)
            ...
            for i, date in enumerate(days):
                ...
                btn = tk.Button(cal_frame, text=summary, command=lambda d=date: show_day_detail(proj, d, content_frame))
                btn.grid(row=0, column=i, padx=5, pady=5)

    proj_list.bind("<<TreeviewSelect>>", show_schedule)

def show_day_detail(proj, date, content_frame):
    ...
    def mark_completed():
        if date in proj.schedule:
            ...
            show_day_detail(proj, date, content_frame)  # Refresh
...
def main_gui():
    ...
    tk.Button(sidebar, text="Add Employee", command=lambda: add_employee_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="Add Equipment", command=lambda: add_equipment_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="View Equipment", command=lambda: view_equipment_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="Create Project", command=lambda: create_project_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="Add Work Item", command=lambda: add_work_item_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="View Projects & Schedules", command=lambda: view_projects_schedules_gui(manager, content_frame)).pack(pady=10, fill="x")
    tk.Button(sidebar, text="View Employees", command=lambda: view_employees_gui(manager, content_frame)).pack(pady=10, fill="x")
