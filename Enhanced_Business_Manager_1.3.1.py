# business_manager.py
# Enhanced with employee availability, scheduling GUI, work library, employee editing, and deletion
...
def add_employee_gui(manager):
    ...
    window = tk.Toplevel()  # Changed to Toplevel for consistency
    ...
    # No mainloop() here; it's a child window

def edit_employee_gui(manager, emp_index):
...
def view_employees_gui(manager):
    if not manager.employees:
        messagebox.showinfo("Employees", "No employees yet.")
        return

    window = tk.Toplevel()
    window.title("View Employees")

    # List of employees
    emp_list = ttk.Treeview(window, columns=("Name", "Role", "Rate"), show="headings")
    ...
    def edit_selected(event):
        ...
        if selected:
            emp_index = int(selected[0])  # iid is the index
            edit_employee_gui(manager, emp_index)
            # Refresh after edit (in case name changes, etc.)
            window.after(500, refresh_list)  # Delay to allow edit window to close
    ...
    tk.Button(window, text="Delete Selected", command=delete_selected).pack(pady=10)

# Placeholder for add_equipment_gui (unchanged; implement as needed)
def add_equipment_gui(manager):
    messagebox.showinfo("Add Equipment", "Equipment addition GUI not implemented yet.")
    # Add your existing code here

# Placeholder for view_equipment_gui (unchanged)
def view_equipment_gui(manager):
    messagebox.showinfo("View Equipment", "Equipment view GUI not implemented yet.")
    # Add your existing code here
...
def main_gui():
    ...
    tk.Button(root, text="View Employees", command=lambda: view_employees_gui(manager)).pack(pady=10)
